...
**Resolved by Matías (2026-09-10):**
- **Mindflayer → Mind-Eater: rename shipped AND merged.** The dungeon boss's literal in-game name was still "Mindflayer" (`assets/voxygen/i18n/en/name.ftl:610-612`, `noun.ftl:23`), unchanged from Veloren, while `xindeler-design`'s `imports/2026-06-21-deity-creature-mapping.md` already had the canon replacement, **"the Mind-Eaters"** (D&D "illithid"/mind flayer is Product Identity). Matías: "Shippea el rename para que quede como el canon. Los nombres tienen que ser de nuestro canon. A menos que no esté, entonces dejamos el que está." — i.e. this is a general rule for every content task below, not just this one creature. Shipped and confirmed merged (per the "Cromatolis - New Horizon" session, 2026-09-10): `xindeler-new-horizon#290` (`rename/mindflayer-to-mind-eaters` → `development`) renames the player-facing display strings in EN + es-419 (`Mind-Eater` / `Devoramentes` — the Spanish rendering is a translation call, not official canon). Internal Rust identifiers and asset paths are untouched (same pattern as Danari/Draugr). **No longer gated** — the wiki's bestiary content (Task 7) can name this creature using the canon name now.
- **Dungeons: two real types, confirmed by Matías** — "Manejamos los dos tipos: el dungeon a través de una misión/historia, y el dungeon que vas explorando por el mundo y te encontrás con uno. Obviamente dependiendo el tier que tengas vas a poder hacerla o no (morís muy rápidamente)." This replaces the "proposal only" treatment below — Task 9 is now a real content task. Two categories: (a) **story/mission dungeons**, tied to quests — tracked as **COW-14** in the game-engine backlog, no spec yet, needs Matías' narrative input (that work happens in `xindeler-new-horizon`/`xindeler-design`, not this wiki); (b) **world-exploration dungeons**, encountered while exploring, gated by a tier/difficulty system where an under-leveled party dies fast rather than being mechanically blocked from entering — tracked as **COW-13**, which already has a spec (`xindeler-design#190`).
  - **Major update (2026-09-10, from the "Cromatolis - New Horizon" session, which owns `xindeler-new-horizon` content work):** world-exploration dungeons **already exist fully built** in the engine — real generators, bosses, and loot for Gnarling, Adlet, Sahagin, Haniwa, Castle, Cultist, Sea Chapel, Terracotta Palace, Dwarven Mine, Troll Cave, Pirate Hideout, Jungle Ruin. Nothing needed porting from Veloren's GitLab. They weren't spawning in-world because of a bug: the wilderness-site placement loop in `civ/mod.rs` zeroes out automatically once authored settlements are loaded, true in Cromatolis since COW-6 merged — COW-13 fixes that, scoped narrowly.
  - **Open question, not this wiki's to decide:** those 12 dungeon names are Veloren's own (not D&D — not canon-lint-denylisted), inherited as-is, and haven't been through the kind of rebrand pass Mindflayer just got. Whether they stay or get renamed is Matías'/Cromatolis' call, not something to assume either way here.
  - **Stat/loot source for the wiki, once this content task runs:** use `xindeler-new-horizon`'s `assets/common/entity/dungeon/` directly, not `wiki.veloren.net` — balance numbers may have changed in this fork (per the Cromatolis session).
- **Magic: add a Sources/Schools/Spells structure section** to `gameplay/magia.md` — Matías' addition, see the "Spells — a major unresolved IP finding" note directly below before treating this as a simple content task.

**⚠️ Spells — a partially-unresolved IP finding (2026-09-10), separate from anything above:**
While verifying the magic system for Matías' new "add a spells section" request, a directory listing of `xindeler-new-horizon`'s `assets/common/abilities/spells/` (451 `.ron` files across folders named after schools — `abjuration/`, `arcane/`, `axiomancy/`, `conjuration/`, `divination/`, `divine/`, `evocation/`, `gravesong/`, `hemomancy/`, `hollow/`, `illusion/`, `primordial/`, `ruin/`, `transmutation/`) showed that **a subset of individual spell names are verbatim D&D 5e spell names**. Matías pushed back (correctly, in part) that many spell names are just generic fantasy-genre terms shared across countless games (Fireball, Heal, Invisibility, Teleport, Haste, Sleep) and not a concern, and that deity/creature copyright names were already handled separately. Both true — but a real subset is distinctly WotC-coined rather than generic, e.g. (discussed with Matías directly in chat, not reproduced in full here to avoid listing it inside a committed file in this public repo): a full family of same-suffix spells tied to one specific class' signature mechanic, one class' single most-iconic cantrip, two cantrips that come from an official 5e supplement rather than the base rules, and a historically D&D-specific naming pattern for a small family of high-level spells. This is the same category of problem `lore/70-style-guide.md`'s denylist already polices for lore prose, but at engine-code scale. As of 2026-09-10 these names are still present verbatim in the live `development` branch (checked directly, not assumed) — if a separate rename pass already covers spells specifically, it hadn't reached this code yet. **Not this wiki session's to fix** — flagged for Matías to route to whoever owns spell content in `xindeler-new-horizon`. **Consequence for the wiki:** the new magic section (Task 11) covers the Source × School *structure* only — no literal spell-name list, ever, until this is resolved. The existing `magia.md` already avoids naming specific spells; keep it that way.

**In progress, not for the wiki yet (mentioned for completeness, no specifics below):** nothing else found in the reviewed window (commits, CHANGELOG, class/race source) that constitutes an unshipped content spoiler — the recent work is overwhelmingly infra/UI/IP-cleanup, not new unreleased game content. No further caution needed beyond the two items above.
...
| Dungeons | *new* `gameplay/mazmorras.md` | Two confirmed types: world-exploration (COW-13, already built in-engine, currently disabled by a placement bug being fixed) and story/mission (COW-14, no spec yet — needs Matías' narrative input). Structure only for now; only name the 12 real world-dungeons if/once Matías confirms their inherited Veloren names stay as-is (open question, see above) — otherwise describe the category without naming instances. |
...
| *(Matías' addition, not from Veloren)* | `gameplay/magia.md` | Add a Sources × Schools × Spells **structure** section — no literal spell-name list, see the IP finding above. |

## Decisions

1. **Scope for this pass**: expand `guias/empezando.md`, `gameplay/combate.md`, `gameplay/crafteo.md`, and `gameplay/magia.md`; add three new pages (`gameplay/armas.md`, `base-de-datos/armaduras.md`, `gameplay/mazmorras.md`); restructure `base-de-datos/criaturas.md` and `npcs.md`.
...
3. **Mindflayer → Mind-Eater**: resolved, see above — gate any use of the name on `xindeler-new-horizon#290` merging.
4. **Dungeons**: resolved, see above — two types, structure-only content, no invented proper nouns.
5. **Spells: never publish a spell-name list.** Structure only (Sources × Schools), until the D&D-name IP finding above is resolved at the engine level. This is not this plan's decision to revisit — treat it as a hard constraint.
6. **i18n**: every task updates both the EN (root) and ES (`es/`) file, per existing repo convention, plus sidebar entries in both `.vitepress/config.mts` locale blocks for new pages.
7. **No deploy**: all work happens on a feature branch off `development`, reviewed locally (`npm run dev`), merged to `development` only after Matías signs off. Never touches `main`/tags.
8. **CLAUDE.md housekeeping**: fix the stale "4 classes" line under Pending Work while touching this area (Task 1).
