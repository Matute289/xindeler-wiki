...
- **Mindflayer → Mind-Eater: rename shipped.** The dungeon boss's literal in-game name was still "Mindflayer" (`assets/voxygen/i18n/en/name.ftl:610-612`, `noun.ftl:23`), unchanged from Veloren, while `xindeler-design`'s `imports/2026-06-21-deity-creature-mapping.md` already had the canon replacement, **"the Mind-Eaters"** (D&D "illithid"/mind flayer is Product Identity). Matías: "Shippea el rename para que quede como el canon. Los nombres tienen que ser de nuestro canon. A menos que no esté, entonces dejamos el que está." — i.e. this is a general rule for every content task below, not just this one creature. Shipped: `xindeler-new-horizon#290` (`rename/mindflayer-to-mind-eaters` → `development`, open for review as of this writing) renames the player-facing display strings in EN + es-419 (`Mind-Eater` / `Devoramentes` — the Spanish rendering is a translation call, not official canon, flagged for Matías in that PR). Internal Rust identifiers and asset paths are untouched (same pattern as Danari/Draugr). **Once that PR merges**, the wiki's bestiary/dungeon content (Tasks 7 and 9) can name this creature using the canon name.
- **Dungeons: two real types, confirmed by Matías** — "Manejamos los dos tipos: el dungeon a través de una misión/historia, y el dungeon que vas explorando por el mundo y te encontrás con uno. Obviamente dependiendo el tier que tengas vas a poder hacerla o no (morís muy rápidamente)." This replaces the "proposal only" treatment below — Task 9 is now a real content task. Two categories: (a) **story/mission dungeons**, tied to quests — this is what `xindeler-design/lore/80-geography/kalthis/wards/.../undercity.md`'s narrative-ward model describes; (b) **world-exploration dungeons**, encountered while exploring, gated by a tier/difficulty system where an under-leveled party dies fast rather than being mechanically blocked from entering. Still never invent specific dungeon/boss names beyond confirmed canon (per the operative rule below) — this decision is about structure, not new proper nouns.
...
**⚠️ Spells — a major unresolved IP finding (2026-09-10), separate from anything above:**
While verifying the magic system for Matías' new "add a spells section" request, a directory listing of `xindeler-new-horizon`'s `assets/common/abilities/spells/` (451 `.ron` files across folders named after schools — `abjuration/`, `arcane/`, `axiomancy/`, `conjuration/`, `divination/`, `divine/`, `evocation/`, `gravesong/`, `hemomancy/`, `hollow/`, `illusion/`, `primordial/`, `ruin/`, `transmutation/`) showed that **a large fraction of individual spell names are verbatim D&D 5e spell names** (well beyond generic terms like "fireball" — many are distinctly D&D-coined, e.g. specific "Smite" family names, "Power Word" family names, signature cantrips, etc.). This is the same category of problem `lore/70-style-guide.md`'s denylist already polices for lore prose, but **at engine-code scale** (hundreds of asset files), not a handful of lore-file mentions. Deliberately **not reproducing the actual names here** — this doc lives in the public `xindeler-wiki` repo, and pasting the list into a committed file would itself be a public IP exposure. This is **flagged for Matías to route to whoever owns IP cleanup in `xindeler-new-horizon`** (the dedicated sessions already working that repo) — fixing it is a large, separate undertaking (renaming spell assets + all their references across combat/UI code), well outside this wiki session's scope. **Consequence for the wiki:** the new magic section (Task 11) covers the Source × School *structure* only — no literal spell-name list, ever, until this is resolved. The existing `magia.md` already avoids naming specific spells; keep it that way.
...
| Dungeons | *new* `gameplay/mazmorras.md` | Two confirmed types (story/mission dungeons vs. world-exploration dungeons with tier-gated difficulty risk) — structure only, no invented boss/dungeon proper nouns beyond confirmed canon. |
